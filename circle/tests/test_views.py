...
from circle.models import Circle, CircleUser, RecentCircle, Policy
...
class TestView(TestCase):
    def setUp(self):
        ...
        self.circle_user_data_2 = CircleUser.objects.create(
            circle_id=self.circle, username=self.userdata_2, is_admin=False, is_member=True
        )
    ...
    def test_current_circle(self):
        response = self.client.post(self.user_circle_url, data = {'remove_user' : "ChinmayKulkarni"})
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "circle/current-circle.html")
    ...
    def test_create_circle(self):
        response = self.client.post(self.create_url, data ={"create_circle" : "", "circle_name" : "TestCircle_2", "policy_id" : "1"})
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "circle/add.html")

    def test_create_circle_withSameName(self):
        response = self.client.post(self.create_url, data ={"create_circle" : "", "circle_name" : "TestCircle", "policy_id" : "1"})
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, "circle/add.html")
